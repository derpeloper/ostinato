 ...
 */
export async function loadTextToSpeech(onnxDir, useGpu = false, gpuProvider = 'cuda') {
    let gpu = false;
    let provider = 'cuda';

    if (typeof useGpu === 'object' && useGpu !== null) {
        gpu = Boolean(useGpu.useGpu);
        provider = useGpu.gpuProvider || useGpu.provider || 'cuda';
    } else {
        gpu = Boolean(useGpu);
    ...
    if (gpu && provider !== 'cpu') {
        sessionOptions.executionProviders = [provider];
        console.log(`[Supertonic] Attempting to load ONNX models with GPU provider: ${provider}...`);
        try {
            sessions = await loadOnnxAll(onnxDir, sessionOptions);
            activeProvider = provider;
            console.log(`[Supertonic] Successfully initialized with ${provider} provider.`);
        } catch (err) {
            console.warn(`[Supertonic] Failed to initialize ${provider} provider, falling back to CPU... (Error: ${err.message})`);
    ...
    textToSpeech.executionProvider = activeProvider;
    textToSpeech.useGpu = activeProvider !== 'cpu';
    textToSpeech.fallback = fallback;
    textToSpeech.fallbackFrom = fallbackFrom;
    textToSpeech.fallbackReason = fallbackReason;
